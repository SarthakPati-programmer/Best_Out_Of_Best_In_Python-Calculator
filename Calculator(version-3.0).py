...
# text_input is a variable being used to print values on calculator screen(see line 37).
# Operator is an empty variable being used to assign new values(see line 35).
def btncleardis():
    global operator
    operator = ""
    text_input.set("")
# This function clears the display(see line 65).
def btndecimal():
    s="."
    global operator
    operator=operator+str(s)
    text_input.set(operator)
#This will implement thw decimal button.
def btnEqualsInput():
        global operator
        try:
...
btn15=Button(cal,padx=16,pady=16,bd=8,fg='blue',font=('aerial',20,'bold'),text="%",bg='Yellow',command=lambda:btnclick("/100" )).grid(row=4,column=2)
